...
import { FakePhoto, Photo, Profile, UserActivity } from "../models/profile";
...
const requests = {
    get: <T> (url: string, options?: { params: URLSearchParams}) => axios.get<T>(url, options).then(responseBody),
...
const Account = {
    ...
    register: (user: UserFormValues) => requests.post<User>('/account/', user),
}

const Profiles = {
    get: (username: string) => requests.get<Profile>(`/profiles/${username}`),
    uploadFakePhoto: (fakePhoto: FakePhoto) => requests.post<Photo>(`/photos/AddFake`, fakePhoto),
